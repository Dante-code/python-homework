...
def load_csc(d):
    ...
    for line in ls:
        ...
        # 重新写入列表dic
    with open('全校老师信息.csv', 'w+', encoding='ANSI') as fw:
        fw.write(','.join(label) + '\n')  # 标题
        for line in dic:
            temp = ''
            for key in label:
...
def main():
    wd = webdriver.Chrome()
    wd.get(
        'https://faculty.xidian.edu.cn/yxsz.jsp?urltype=tree.TreeTempUrl&wbtreeid=1020'
    )
    labels = wd.find_elements_by_css_selector('.subject .li-b ul li a')
    ...
    facultyURL = [labela.get_attribute('href') for labela in labels]

    try:
        for url in facultyURL:
            wd.get(url)
            while True:
